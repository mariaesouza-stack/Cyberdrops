...
@Component({
  ...
  template: `<section class="page profile-page"><div class="profile-hero"><a class="avatar profile-avatar" routerLink="/avatars">{{ service.user().avatar }}<span>✎</span></a><h1>{{ service.user().name }}</h1><p>{{ service.user().username }}</p><div><b>18</b><span>Drops</span><b>2.4k</b><span>Fogo recebido</span></div></div>
    <div class="tabs"><button [class.active]="tab() === 'edit'" (click)="tab.set('edit')">Editar informações</button><button [class.active]="tab() === 'notifications'" (click)="tab.set('notifications')">Notificações</button></div>
    @if (tab() === 'edit') { <form class="profile-form"><label>Nome<input [(ngModel)]="draft.name" name="name"></label><label>Username<input [(ngModel)]="draft.username" name="username"></label><label>E-mail<input [(ngModel)]="draft.email" name="email"></label><label>Telefone<input [(ngModel)]="draft.phone" name="phone"></label><label>Senha<input type="password" value="123456789"></label><button type="button" class="button primary wide" (click)="save()">Salvar alterações</button></form>
...
export class ProfilePage {
  readonly service = inject(UserService); readonly tab = signal<'edit'|'notifications'>('edit'); draft = { ...this.service.user() };
