...
@Component({
  standalone: true, imports: [FormsModule, RouterLink, OfferCardComponent, AppIconComponent],
  template: `<section class="page profile-page"><div class="profile-hero"><a class="avatar profile-avatar" routerLink="/avatars" aria-label="Selecionar avatar"><app-icon [name]="service.user().avatar" [size]="44"/><span><app-icon name="pencil" [size]="14"/></span></a><h1>{{ service.user().name }}</h1><p>{{ service.user().username }}</p><div><b>18</b><span>Drops</span><b>2.4k</b><span>Fogo recebido</span></div></div>
