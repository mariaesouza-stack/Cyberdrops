...
@Component({
  standalone: true, imports: [RouterLink, AvatarPickerComponent, AppIconComponent],
  template: `<section class="page avatar-page"><a class="back" routerLink="/profile" aria-label="Voltar"><app-icon name="arrow-left" [size]="20"/></a><p class="eyebrow">IDENTIDADE VISUAL</p><h1>Escolha seu avatar</h1><p class="muted">Mostre seu estilo para a comunidade.</p><div class="avatar-preview avatar"><app-icon [name]="selected()" [size]="52"/></div><app-avatar-picker [selected]="selected()" (picked)="selected.set($event)"/><button class="button primary wide" (click)="save()">Salvar avatar</button></section>`
})
