...
@Component({
  standalone: true, imports: [CurrencyPipe, FormsModule, RouterLink, CouponCardComponent, CommentCardComponent],
  template: `@if (offer(); as item) { <main class="product page">
    ...
    <section class="product-info"><div class="badges"><span>{{ item.store }}</span><span>{{ item.category }}</span></div><h1>{{ item.title }}</h1><p>{{ item.description }}</p><div class="price"><small>{{ item.oldPrice | currency:'BRL' }}</small><strong>{{ item.price | currency:'BRL' }}</strong></div>
      <button class="button primary wide" (click)="service.openStore(item)">Ir para a loja ↗</button>@if (item.coupon) { <app-coupon-card [coupon]="item.coupon"/> }
      <div class="author"><span class="avatar">{{ item.author.avatar }}</span><div><small>PUBLICADO POR</small><b>{{ item.author.name }}</b></div><div class="offer-actions"><button (click)="service.vote(item.id, 'like')">🔥 {{ item.likes }}</button><button (click)="service.toggleSaved(item.id)">{{ item.saved ? 'Salvo' : 'Salvar' }}</button></div></div>
    </section>
    <section class="product-detail"><div class="tabs pill-tabs"><button [class.active]="tab() === 'discussion'" (click)="tab.set('discussion')">Discussão</button><button [class.active]="tab() === 'about'" (click)="tab.set('about')">Sobre o jogo</button></div>
      @if (tab() === 'discussion') { <div class="discussion"><div class="comment-composer"><span class="avatar">{{ user.user().avatar }}</span><textarea [(ngModel)]="commentText" placeholder="O que você achou deste drop?"></textarea><button class="button primary" (click)="comment(item.id)">Comentar</button></div>@for (comment of item.comments; track comment.id) { <app-comment-card [comment]="comment" (replied)="reply(item.id, $event)"/> }</div>
