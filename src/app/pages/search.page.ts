...
@Component({
  standalone: true, imports: [FormsModule, CouponOfferCardComponent, OfferCardComponent, RouterLink],
  template: `<section class="page search-page"><p class="eyebrow">SCAN GLOBAL</p><h1>O que você procura?</h1>
    <label class="search-box"><span class="search-icon">⌕</span><input [ngModel]="query()" (ngModelChange)="search($event)" (focus)="focused.set(true)" (blur)="focused.set(false)" placeholder="Busque por produtos, cupons e lojas">@if (focused() && query().length > 0) { <button type="button" (mousedown)="$event.preventDefault()" (click)="clear()">×</button> }</label>
    ...
    } @else { <div class="discover"><section><div class="section-heading"><h2>Lojas populares</h2></div><div class="popular-stores">@for (store of service.stores.slice(0, 3); track store.id) { <button><i [style.background]="store.color">{{ store.icon }}</i><b>{{ store.name }}</b></button> }</div></section>
