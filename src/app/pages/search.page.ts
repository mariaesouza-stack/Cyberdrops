...
@Component({
  standalone: true, imports: [FormsModule, CouponOfferCardComponent, OfferCardComponent, RelatedProductCardComponent, AppIconComponent],
  template: `<section class="page search-page"><p class="eyebrow">SCAN GLOBAL</p><h1>O que você procura?</h1>
    ...
    @if (query()) { <div class="search-results"><h2>Resultados para “{{ query() }}”</h2>@for (offer of results(); track offer.id) { <app-offer-card [offer]="offer"/> } @empty { <div class="empty"><app-icon name="search" [size]="36"/><b>Nenhum drop encontrado.</b><span>Tente buscar por Steam, games ou hardware.</span></div> }</div>
      ...
      <section><div class="section-heading"><h2>Cupons populares</h2></div><div class="coupon-scroll">@for (offer of couponOffers(); track offer.id) { <app-coupon-offer-card [offer]="offer" [showLike]="false" [showShare]="false" [compactCopy]="true"/> }</div></section>
      <section><div class="section-heading"><h2>Produtos populares</h2></div><div class="related search-related-products">@for (product of popularProducts(); track product.id) { <app-related-product-card [product]="product"/> }</div></section></div> }
  </section>`
})
export class SearchPage {
  ...
  readonly popularProducts = computed(() => this.service.offers().filter(offer => !offer.coupon).slice(0, 3));
