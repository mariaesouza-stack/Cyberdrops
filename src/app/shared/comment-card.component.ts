...
@Component({
  ...
  template: `<article class="comment">
    ...
    <div><strong>{{ comment().user.name }}</strong><small>{{ comment().time }}</small><p>{{ comment().text }}</p>
      <button>♡ {{ comment().likes }}</button><button (click)="toggleReply()">Responder</button>
...
export class CommentCardComponent {
  readonly comment = input.required<Comment>(); readonly replied = output<{ commentId: number; text: string }>();
