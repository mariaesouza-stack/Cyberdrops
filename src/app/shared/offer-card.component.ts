...
@Component({
  ...
  template: `<article class="offer-card" role="link" tabindex="0" (click)="open()" (keydown.enter)="open()" (keydown.space)="open($event)">
    <header class="offer-user"><span class="avatar mini">{{ offer().author.avatar }}</span><div><strong>{{ offer().author.name }}</strong><small>{{ offer().store }} · {{ offer().time }}</small></div><button aria-label="Compartilhar oferta" (click)="share($event)">{{ shared() ? '✓' : '↗' }}</button></header>
