...
import { PublicationDraft, PublicationType } from '../models';
...
@Component({
  ...
  template: `<div class="publication-overlay" (click)="closed.emit()"><section class="publication-sheet" role="dialog" aria-modal="true" aria-label="Criar publicação" (click)="$event.stopPropagation()">
    <header><div><p class="eyebrow">COMPARTILHE UM DROP</p><h2>{{ type() ? (type() === 'coupon' ? 'Publicar cupom' : 'Publicar item em promoção') : 'O que você quer publicar?' }}</h2></div><button aria-label="Fechar" (click)="closed.emit()"><app-icon name="x" [size]="20"/></button></header>
    ...
    } @else { <button class="publication-back" (click)="type.set(undefined)"><app-icon name="arrow-left" [size]="16"/>Voltar às opções</button>@if (type() === 'coupon') { <app-create-coupon-form (submitted)="submitted.emit($event)"/> } @else { <app-create-deal-item-form (submitted)="submitted.emit($event)"/> } }
  </section></div>`
})
